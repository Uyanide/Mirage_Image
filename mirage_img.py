...
class Mirage_Image_Colored:
    def load_inner_image(self, image_path, max_size) -> None:
        '''
        加载里图, 限制最大尺寸, 进行缩放
    ...
    def load_cover_image(self, image_path, target_size) -> None:
        '''
        加载表图, 根据里图尺寸裁剪并缩放
        :param image_path: 图片路径
    ...
    def merge(self, limit_inner, limit_cover, hiding_rate) -> None:
        '''
        生成隐写图
        ...
        self.output_img = Image.new('RGB', self.inner_img.size)
        pixels_output = self.output_img.load()
    ...
    def __init__(self, inner_path, cover_path, max_size, limit_inner, limit_cover, hiding_rate):
        ...
        self.merge(limit_inner, limit_cover, hiding_rate)

    def save(self, output_path, format, quality, subsampling) -> None:
        '''
        保存隐写图
        ...
        '''
        self.output_img.save(output_path, format, quality = quality, subsampling = subsampling)
